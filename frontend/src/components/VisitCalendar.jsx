...
export default function VisitCalendar({ year, month, dayData }) {
      ...
      <div className="visit-calendar__grid">
        {cells.map((c, i) => {
          ...
          return (
            <div
              key={c.key}
              className={`visit-calendar__cell ${c.count > 0 ? 'has-visit' : ''} ${isToday ? 'is-today' : ''}`}
              style={c.valor > 0 ? { background: `rgba(34, 197, 94, ${intensity})` } : undefined}
              title={c.count > 0 ? `${c.count} visita${c.count === 1 ? '' : 's'} — R$ ${money(c.valor)}` : ''}
            >
