...
function sendVisitApprovalEmail(request, company) {
  ...
  sendMail({
    ...
    text: `Olá,\n\nUma visita técnica foi agendada para "${request.descricao}".\n\nPara autorizar a visita, acesse o link abaixo e faça login na sua conta:\n${link}\n\nEste link expira em 14 dias.`,
    html: actionEmailHtml({
      title: 'Autorizar visita técnica',
      message: `Uma visita técnica foi agendada referente ao chamado <strong>#${request.numero}</strong>: "${request.descricao}". Clique no botão abaixo e faça login na sua conta para autorizar (ou recusar) a visita.`,
