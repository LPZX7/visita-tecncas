...
router.post('/', requireRole('cliente', 'analista', 'gestor'), (req, res) => {
  ...
  const equipment = db.getEquipmentById(equipamento_id);
  ...
  const company = db.getCompanyById(empresa_id);
  if (company?.email) {
    sendMail({
      to: company.email,
...
router.patch('/:id', requireRole('tecnico', 'analista', 'gestor'), (req, res) => {
  ...
  if (req.user.role === 'tecnico') {
    ...
    if (assigned_technician !== undefined) {
      if (assigned_technician) {
        const tech = db.getUserById(assigned_technician);
  ...
  const updated = db.updateRequest(request.id, patch);

  if (patch.status) {
    const company = db.getCompanyById(updated.empresa_id);
    if (company?.email) {
      sendMail({
        to: company.email,
...
router.patch('/:id/avaliacao', requireRole('cliente'), (req, res) => {
  const request = db.getRequestById(req.params.id);
  if (!request) {
    return res.status(404).json({ error: 'Solicitação não encontrada' });
  }
...
router.get('/:id/relatorio-pdf', (req, res) => {
  const request = db.getRequestById(req.params.id);
  if (!request) {
    return res.status(404).json({ error: 'Solicitação não encontrada' });
  }
  ...
  const equipment = db.getEquipmentById(request.equipamento_id);
  const technician = request.assigned_technician ? db.getUserById(request.assigned_technician) : null;
